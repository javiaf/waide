...
export async function getListOfQueues(skipCache: boolean = false) {
  ...
  "pageSize": 50, // Number | Page size
    ...
    } else if (cache['orgqueues']){
        return cache['orgqueues'];
    } else {
        try {
            cache['orgqueues'] = await routingApi.getRoutingQueues();
            return cache['orgqueues'];
        } catch (err) {
            console.error(err)
        }
